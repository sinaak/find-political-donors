...
class Donation:
    ...
    def update_median(self):
        if Donation.comparison_mode == 1:
            self.zip_median = DesideRounding(
                self.zip_amount / self.number_of_zip_donations)
        elif Donation.comparison_mode == 2:
            self.date_median = DesideRounding(
                self.date_amount / self.number_of_date_donations)
    ...
    def copy(self):
        ret = Donation(self.cmte_id, self.zip_code,
                       self.transaction_dt, self.transaction_amt, self.other_id)
